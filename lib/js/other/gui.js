...
function windowingInitialize(elId, canvasId ) {
	cout("windowingInitialize() called.", 0);
	windowStacks[0] = windowCreate(elId, true);
...
function registerGUIEvents() {
	...
	addEvent("click", document.getElementById("data_uri_clicker"), function () {
		...
		if (datauri != null && datauri.length > 0) {
			try {
				...
				initPlayer();
				start(canvas, base64_decode(datauri));
			}
			catch (error) {
		...
		if (GameBoyEmulatorInitialized()) {
			try {
				if (!gameboy.fromSaveState) {
					initPlayer();
					start(canvas, gameboy.getROMImage());
				}
				else {
					initPlayer();
					openState(gameboy.savedStateFileName, canvas);
				}
			}
			catch (error) {
				alert(error.message + " file: " + error.fileName + " line: " + error.lineNumber);
	...
	initPlayer();
	start(canvas, base64_decode(zeldaRomData));
